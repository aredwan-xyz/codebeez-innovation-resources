# Project Showcase

> Part of **[CodeBeez Ultimate Innovation & Technology Resources](../README.md)** by [CodeBeez Innovation](https://codebeez.xyz) · [Abid Redwan](https://aredwan.com)

Community-built AI projects using the resources in this repository.

> Want your project featured? Open a PR following the template below.

---

## Featured Projects

### Developer Tools

| Project | Description | Stack | Author |
|---------|-------------|-------|--------|
| [Add yours!](#contributing) | Your AI project here | ... | You |

### Research & Education

| Project | Description | Stack | Author |
|---------|-------------|-------|--------|
| [Add yours!](#contributing) | Your AI project here | ... | You |

### Creative AI

| Project | Description | Stack | Author |
|---------|-------------|-------|--------|
| [Add yours!](#contributing) | Your AI project here | ... | You |

---

## Contributing Your Project

To add your project, open a PR that adds a row to the appropriate table using this format:

```markdown
| [Project Name](https://github.com/aredwan-xyz) | One-line description | Python, Anthropic API | [@aredwan-xyz](https://github.com/aredwan-xyz) |
```

**Requirements:**
- Open source (public GitHub/GitLab repo)
- Uses at least one resource from this repo
- README in the repository
- Actively maintained

---

*[Back to main README](../README.md)*
